...
import { FixtureModelAdapter, FixtureSearchAdapter } from "./adapters.js";
import { outputSchema, validateEvent, validateInput, validateOutput } from "./contracts.js";
import { documentQaInstructions } from "./prompt.js";
...
export interface ScenarioExecution {
  ...
  modelAdapter: FixtureModelAdapter;
}
...
export const runScenario = async (
  ...
  const modelAdapter = new FixtureModelAdapter(scenario);
...
export const checkConformance = (execution: ScenarioExecution): string[] => {
  ...
  if (modelAdapter.requests.length === 1 && scenario.search.behavior === "return") {
    ...
    if (request?.instructions !== documentQaInstructions) {
      failures.push("model did not receive the document Q&A instructions");
    }
    if (!isDeepStrictEqual(request?.response_schema, outputSchema)) {
      failures.push("model did not receive output.schema.json as its response contract");
    }
